...
# Disable TorchDynamo graph capture to avoid interaction issues with DeepSpeed
# ZeRO-3 parameter partitioning (seen as "tracing error at step ..." in logs).
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # FlashVID / dual-path KD arguments
    enable_flashvid: bool = field(default=False)
    flashvid_retention_ratio: float = field(default=0.1)
    ...
    flashvid_enable_llm_compression: bool = field(default=False)

    dualpath_kd: bool = field(default=False)
    kd_loss_weight: float = field(default=1.0)
    ce_loss_weight: float = field(default=1.0)
    kd_temperature: float = field(default=1.0)
    kd_warmup_ratio: float = field(default=0.0)
    kd_topk: int = field(default=0)
    kd_teacher_confidence_threshold: float = field(default=0.0)
    kd_only_teacher_correct: bool = field(default=True)
    kd_gt_confidence_threshold: float = field(default=0.0)
    kd_max_ce_ratio: float = field(default=1.0)
    kd_margin_loss_weight: float = field(default=0.0)
    kd_margin_huber_delta: float = field(default=1.0)
    teacher_disable_lora: bool = field(default=True)
    lora_include_mm_projector: bool = field(default=True)
...
def find_all_linear_names(model, include_mm_projector: bool = False):
    cls = torch.nn.Linear
    lora_module_names = set()
    extra_target_module_names = set()
    multimodal_keywords = ["mm_projector", "vision_tower", "vision_resampler"]
    for name, module in model.named_modules():
        if isinstance(module, cls):
            if any(mm_keyword in name for mm_keyword in multimodal_keywords):
                if include_mm_projector and "mm_projector" in name:
                    extra_target_module_names.add(name)
                continue
            names = name.split(".")
    ...
    return list(sorted(lora_module_names | extra_target_module_names))
...
def preprocess_llama3(
    ...
    # Automatically add the BOS token ID when required.
...
def maybe_import_flashvid():
    try:
        ...
        if str(flashvid_root) not in sys.path:
            sys.path.insert(0, str(flashvid_root))
        from flashvid import flashvid
        return flashvid


def maybe_enable_flashvid(model, training_args):
    if not (training_args.enable_flashvid or training_args.dualpath_kd):
        return model
    flashvid = maybe_import_flashvid()
    return flashvid(
        model,
        retention_ratio=training_args.flashvid_retention_ratio,
        do_segment=training_args.flashvid_do_segment,
        segment_threshold=training_args.flashvid_segment_threshold,
        min_segment_num=training_args.flashvid_min_segment_num,
        complementary_segment=training_args.flashvid_complementary_segment,
        token_selection_method=training_args.flashvid_token_selection_method,
        alpha=training_args.flashvid_alpha,
        temporal_threshold=training_args.flashvid_temporal_threshold,
        expansion=training_args.flashvid_expansion,
        pruning_layer=training_args.flashvid_pruning_layer,
        llm_retention_ratio=training_args.flashvid_llm_retention_ratio,
        enable_llm_compression=training_args.flashvid_enable_llm_compression,
    )
...
def train(attn_implementation=None):
    ...
    if training_args.dualpath_kd and not training_args.enable_flashvid:
        raise ValueError("dualpath_kd=True requires --enable_flashvid True.")
    if training_args.dualpath_kd and not training_args.lora_enable:
        rank0_print("[WARN] dualpath_kd is enabled without LoRA. This is supported, but it is not the intended setup.")
    ...
    model = maybe_enable_flashvid(model, training_args)
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(model, include_mm_projector=training_args.lora_include_mm_projector),
