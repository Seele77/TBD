...
MODEL_NAME=${MODEL_NAME:-lmms-lab/llava-onevision-qwen2-7b-ov}
DATA_PATH=${DATA_PATH:-./data/llava_video_178k/train.json}
VIDEO_FOLDER=${VIDEO_FOLDER:-./data/llava_video_178k/videos}
OUTPUT_DIR=${OUTPUT_DIR:-./outputs/anonymous_experiment_onevision}
DEEPSPEED_CFG=${DEEPSPEED_CFG:-./scripts/zero2.json}
FRAMES_UPBOUND=${FRAMES_UPBOUND:-32}
VIDEO_FPS=${VIDEO_FPS:-1}

# The appendix recipe targets the main LLaVA-Video experiment. This released
# OneVision launcher keeps the margin term disabled for its reported run.
