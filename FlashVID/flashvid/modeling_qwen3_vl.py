...
def Qwen3VLModel_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    enable_visual_compression = bool(flashvid_enabled and getattr(flashvid_config, "enable_visual_compression", True))
    return_compression_metadata = bool(getattr(flashvid_config, "return_compression_metadata", False))
    if (
        flashvid_config is not None
        and pixel_values_videos is not None
        and "cls_attention" in locals()
        and position_ids.shape[-1] > 1
        and enable_visual_compression
    ):
        ...
        if return_compression_metadata:
            compressed_video_tokens, keep_visual_global_indices, compression_metadata = flashvid_compression(
                video_features=video_features,
                cls_attention=cls_attention,
                flashvid_config=flashvid_config,
                return_metadata=True,
            )
            setattr(self, "flashvid_last_meta", compression_metadata)
        else:
            ...
            setattr(self, "flashvid_last_meta", None)
        ...
        deepstack_visual_embeds = [
            deepstack_visual_embed[keep_visual_global_indices] for deepstack_visual_embed in deepstack_visual_embeds
        ]
        keep_global_indexes = (
            torch.cat(
                [
        ...
        inputs_embeds[:, visual_token_indexes[keep_visual_global_indices]] = compressed_video_tokens.view(
            -1, hidden_size
        ).unsqueeze(0)
