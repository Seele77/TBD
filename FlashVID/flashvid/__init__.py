...
def flashvid(
    ...
    Args:
        ...
        enable_llm_compression (bool, optional): Whether to enable inner-LLM pruning. Defaults to False.
    ...
    flashvid_config = FlashVidConfig(
        ...
        enable_llm_compression=enable_llm_compression,
    )

    # Store FlashVid Config in the model.
    setattr(model, "flashvid_config", flashvid_config)
    setattr(model, "flashvid_enabled", True)
    setattr(model, "flashvid_last_meta", None)
    setattr(model.model, "flashvid_config", flashvid_config)
    setattr(model.model, "flashvid_enabled", True)
    if type(model) in (Qwen2_5_VLForConditionalGeneration, Qwen3VLForConditionalGeneration):
        ...
        setattr(model.model.language_model, "flashvid_enabled", True)
